...
(define test-1-wire-1
  "R75,D30,R83,U83,L12,D49,R71,U7,L72")
(define test-1-wire-2
  "U62,R66,U55,R34,D71,R55,D58,R83")
;; distance = 159
(define test-2-wire-1
  "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51")
(define test-2-wire-2
  "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7")
;; distance = 135


; takes a string of instructions like      : "U98,R91,D20"
; turns it into a list of instructions like: ("U98" "R91" "D20")
(define (split-into-instructions p)
  (string-split p ","))


; Gets the manhattan distance (taxicab distance)
; between two points (2 2) (0 0)
; in this case 4
(define (manhattan-distance pair-1 pair-2)
  (+ (abs (- (first pair-1)
             (first pair-2)))
     (abs (- (second pair-1)
             (second pair-2)))))
...
; turns a list of instructions
; (list (list 0 0)) (list "U98" "R91" "D20")
; into:
; a list of corner coordinates (of the path):
; '((91 -78) (91 98) (0 98) (0 0))
(define (instructions->corners path instructions)
  (if (empty? instructions)
      path
      (instructions->corners
       (cons (interpret-instruction
              (car path)
              (car instructions))
             path)
       (cdr instructions))))
...
; turns an instruction : "U98"
; and a position (list 0 0)
; into a new position: (list 0 98)
(define (interpret-instruction current-pos instruction)
  (let ([direction (substring instruction 0 1)])
    (cond [(string=? direction "U")
            (handle-up current-pos instruction)]
           [(string=? direction "R")
            (handle-right current-pos instruction)]
           [(string=? direction "D")
            (handle-down current-pos instruction)]
           [(string=? direction "L")
            (handle-left current-pos instruction)])))

(define (handle-up pos instruction)
  (let ([x (first pos)]
        [y (+ (string->number (substring instruction 1))
              (second pos))])
    (list x y)))

(define (handle-left pos instruction)
  (list
   (+ (string->number (substring instruction 1))
      (first pos))
   (second pos)))

(define (handle-down pos instruction)
  (list
   (first pos)
   (- (string->number (substring instruction 1))
      (second pos))))

(define (handle-right pos instruction)
  (list
   (+ (string->number (substring instruction 1))
      (first pos))
   (second pos)))
...
; given two points (or coordinates)
; where either the x or y coordinate is the same
; list the points that lay in between them
(define (list-between-points point-1 point-2)
  (let ([first-changes (not (= (first point-1)
                               (first point-2)))]
        [second-changes (not (= (second point-1)
                                (second point-2)))])
    (cond [first-changes
           (score-x (first point-1)
                    (first point-2)
                    (second point-1))]
          [second-changes
           (score-y (second point-1)
                    (second point-2)
                    (first point-1))])))

(define (score-x start stop constant)
  (map (lambda (x) (list x constant))
       (inbetween start stop)))

(define (score-y start stop constant)
  (map (lambda (y) (list constant y))
       (inbetween start stop)))

(define (inbetween from to)
  (cond [(= from to)
         (list)]
        [(< from to)
         (cons from
...
; this takes a thousand years to execute
(define (complete-paths path)
  (if (empty? (cdr path))
      (append path (list))
      (append (list-between-points (car path) (cadr path))
              (complete-paths (cdr path)))))


; turn a list of pairs of corners into a list of points between those corners
; so points between corner 1 and corner 2
; the points between corner 2 and corner 3
; ect.
(define (corner-pairs->lines list-of-corner-pairs)
  (if (empty? (cdr list-of-corner-pairs))
       (list (list-between-points (caar list-of-corner-pairs)
                                 (cadar list-of-corner-pairs)))
      (cons (list-between-points (caar list-of-corner-pairs)
                                 (cadar list-of-corner-pairs))
            (corner-pairs->lines (cdr list-of-corner-pairs)))))

(define (pair-corners list-of-corners)
  (if (empty? (cdr list-of-corners))
      (list)
      (cons (list (car list-of-corners)
                  (cadr list-of-corners))
            (pair-corners (cdr list-of-corners)))))

(define actual-testwire-1-1 
(corner-pairs->lines
 (pair-corners
  (instructions->corners
   (list (list 0 0))
   (split-into-instructions test-1-wire-1)))))

(define actual-testwire-1-2
(corner-pairs->lines
 (pair-corners
  (instructions->corners
   (list (list 0 0))
   (split-into-instructions test-1-wire-2)))))

; look for item (list 297 -57)
; in list of items (list (299 -57) (298 -57) (297 -57))
; gives back item if found else gives back the item
(define (look item line)
  (findf
   (lambda (x)
     (and (= (first x) (first item))
          (= (second x) (second item))))
   line))

(look (list 297 -57) (car actual-testwire-1-1))
(car actual-testwire-1-1)
